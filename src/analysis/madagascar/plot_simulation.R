...
load(file = here("results", "88qlclkf.rda"))
...
plt.1 = ggplot(tmp, aes(x = method, y = value, fill = cvstrat)) +
  ...
  scale_fill_manual(values = viridis(n = 2, alpha = 1, begin = 0.3, end = 0.7)) +
  facet_grid(variable ~ holdout, scales = "free") +
  theme(legend.position = "bottom")

ggsave(plot = plt.1, filename = "fig_simresults_mdg.png", path = here("figures"), dpi = 600, scale = 0.9, width = 210, height = 297, units = "mm", device = png)
